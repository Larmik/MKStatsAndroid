...
class WarLocalDataSource @Inject constructor(@ApplicationContext private val context: Context) :
    ...
    override fun insert(wars: List<MKWar>): Flow<Unit> = flow { emit(dao.bulkInsert(wars.mapNotNull { it.war?.toEntity(it.name) })) }

    override fun insert(war: MKWar): Flow<Unit> = flow {
        war.war?.toEntity(war.name)
            ?.let { dao.insert(it) }?.let { emit(it) }
    }

    override fun delete(war: MKWar): Flow<Unit> = flow {
        war.war?.toEntity(war.name)
            ?.let { dao.delete(it) }?.let { emit(it) }
    }
