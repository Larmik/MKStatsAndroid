...
import fr.harmoniamk.statsmk.fragment.settings.manageTeams.EditTeamViewModel
import kotlinx.coroutines.ExperimentalCoroutinesApi
import kotlinx.coroutines.FlowPreview

@OptIn(FlowPreview::class, ExperimentalCoroutinesApi::class, ExperimentalMaterialApi::class)
@Composable
fun EditTeamScreen(viewModel: EditTeamViewModel = hiltViewModel(), teamId: String, onDismiss: () -> Unit) {
    ...
    MKBaseScreen(title = "Edition équipe", subTitle = team.value?.name.orEmpty()) {
        MKTextField(value = nameState.value, onValueChange = { nameState.value = it }, placeHolderRes = R.string.modifier_le_nom, keyboardType = KeyboardType.Text)
        MKTextField(value = tagState.value, onValueChange = { tagState.value = it }, placeHolderRes = R.string.modifier_le_tag, keyboardType = KeyboardType.Text)
        MKButton(text = R.string.enregistrer, enabled = enabled) { viewModel.onTeamEdited(nameState.value.text, tagState.value.text) }
    }
}