...
@Composable
fun MKPlayerScoreStatsView(stats: Stats) {
    Column(Modifier.padding(bottom = 20.dp)) {
        MKText(text = "Scores", fontSize = 16, font = R.font.montserrat_bold, modifier = Modifier.padding(bottom = 10.dp))
        Row {
            Column(Modifier.weight(1f), horizontalAlignment = Alignment.CenterHorizontally) {
                MKText(text = R.string.pire_score, fontSize = 12)
                MKText(text = stats.lowestPlayerScore?.first?.toString() ?: stats.lowestScore?.score.toString(), fontSize = 20, font = R.font.orbitron_semibold)
            ...
            Column(Modifier.weight(1f), horizontalAlignment = Alignment.CenterHorizontally) {
                ...
                MKText(text = stats.highestPlayerScore?.first?.toString() ?: stats.highestScore?.score.toString(), fontSize = 20, font = R.font.orbitron_semibold)
